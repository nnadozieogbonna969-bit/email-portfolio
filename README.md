# 📧 Email Portfolio

Welcome to my Email Design Portfolio — a collection of high-converting, responsive, and visually engaging custom HTML email projects built for real-world marketing use cases.

This portfolio showcases my skills in:
- HTML email development (table-based + modern inline CSS)
- Responsive email design
- Marketing-focused copy structuring
- Conversion-oriented layout design
- Brand-consistent email systems

---

## 🚀 Live Portfolio

🔗 Main Portfolio:
https://nnadozieogbonna969-bit.github.io/email-portfolio/

## 📁 Projects Included

### 💼 Brand Coach Email
A high-conversion email designed for coaching and personal development brands focused on authority positioning and email list engagement.

🔗 View Project:
https://nnadozieogbonna969-bit.github.io/email-portfolio/brand-coach-email/

---

### 🏋️ Fitness Email
A bold, energetic promotional email designed for fitness offers, training programs, and product sales.

🔗 View Project:
https://nnadozieogbonna969-bit.github.io/email-portfolio/fitness-email/

---

### 🌸 Perfume Brand Sales Email
A luxury-style promotional email designed for fragrance and lifestyle brands, focused on emotional selling and visual appeal.

🔗 View Project:
https://nnadozieogbonna969-bit.github.io/email-portfolio/perfume-brand-sales/

---

## 🛠️ Tech Stack

- HTML5
- Inline CSS (email-safe styling)
- VML (for Outlook compatibility where needed)
- Basic JavaScript (for portfolio interactivity where applicable)
- GitHub Pages (hosting)

---

## 🎯 Purpose of This Portfolio

This portfolio was built to demonstrate:
- Real-world email marketing design skills
- Conversion-focused copy and layout strategy
- Brand storytelling through email
- Ability to build production-ready email templates

---

## 📌 Notes

- All emails are fully optimized for the major email clients.
- Designs follow best practices for deliverability and engagement.
- This portfolio is continuously updated with new projects.

---

## 📬 Contact

If you'd like to collaborate or hire me for email design or frontend work, feel free to reach out.

---
